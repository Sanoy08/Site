...
// ★ New Imports for Native/Web Google Auth
...
export default function LoginPage() {
  ...
  // ★ Updated Google Login Handler (Native + Web Compatible)
  const handleGoogleLogin = async () => {
    setIsLoading(true);
    try {
      // 1. Perform Client-Side Login (Popup on Web, Native Sheet on Mobile)
      const googleUser = await GoogleAuth.signIn();
      
      console.log("Google User:", googleUser);

      // 2. Send the ID Token to your backend for verification & session creation
      ...
      // 3. Log the user in with your app's JWT
      login(data.user, data.token);
