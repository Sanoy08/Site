    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 pb-20 px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Customer Details */}
                <Card className="md:col-span-1 shadow-sm h-fit border-0 bg-muted/20">
                    <CardHeader><CardTitle className="text-lg">Customer Info</CardTitle></CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            <Label>Full Name</Label>
                            <Input value={customerInfo.name} onChange={e => setCustomerInfo({...customerInfo, name: e.target.value})} placeholder="Customer Name" className="h-11 rounded-xl" />
                        </div>
                        <div className="space-y-2">
                            <Label>Address (Required)</Label>
                            <Input value={customerInfo.address} onChange={e => setCustomerInfo({...customerInfo, address: e.target.value})} placeholder="Full Address" className="h-11 rounded-xl" />
                        </div>
                        <div className="space-y-2">
                ...
                <Card className="md:col-span-2 shadow-sm border-0 bg-card">
                    <CardHeader className="flex flex-row items-center justify-between border-b pb-4">
                        ...
                        <Button variant="outline" size="sm" onClick={handleAddItem} className="gap-1 rounded-full px-4 border-primary/30 text-primary">
                    ...
                    <CardContent className="p-0">
                            ...
                            {items.map((item) => (
                                <div key={item.id} className="p-4 space-y-4 animate-in fade-in zoom-in-95 duration-200">
                                    <div className="space-y-2">
                                        ...
                                        <select 
                                            className="flex h-11 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm ring-offset-background focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 appearance-none"
                                            value={item.name}
                                        ...
                                        {/* যদি ড্রপডাউনে না থাকে, তবে ম্যানুয়ালি নাম লেখার ইনপুট */}
                                        {item.name === "custom" && (
                                            <Input 
                        ...
                        <div className="p-6 bg-muted/10 border-t space-y-4">
                            <div className="space-y-3 bg-white p-4 rounded-2xl border border-dashed border-primary/20">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <Input type="number" value={discount || ''} onChange={e => setDiscount(Number(e.target.value))} className="w-24 h-9 text-right rounded-lg" />
                                </div>
                                <div className="flex items-center justify-between">
                                    <Label className="text-sm">Received Amount (₹)</Label>
                                    <Input type="number" value={receivedAmount || ''} onChange={e => setReceivedAmount(Number(e.target.value))} className="w-24 h-9 text-right rounded-lg" />
                                </div>
                            ...
                            <Button onClick={handleDownloadInvoice} disabled={isGenerating} className="w-full h-14 text-lg rounded-2xl shadow-lg shadow-primary/20">
