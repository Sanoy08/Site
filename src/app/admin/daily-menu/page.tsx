...
import { Tabs, TabsList, TabsTrigger } from '@/components/ui/tabs'; // ★ Tabs Import
...
export default function DailyMenuPage() {
  ...
  // ★ নির্দিষ্ট ডিশ স্টেটে লোড করার ফাংশন
  const loadDishToState = (dish: any, index: string) => {
    setActiveTabIndex(index);
    setCurrentDishId(dish._id || "");
    setName(dish.name || `Special Dish ${parseInt(index) + 1}`);
    setPrice(dish.price || "");
    setInStock(dish.inStock ?? true);
    setHistory([]); // ট্যাব বদলালে হিস্ট্রি ক্লিয়ার
    
    if (dish.description) {
        const extractedItems = dish.description.split('\n')
            .map((line: string) => line.replace(/^•\s*/, '').trim())
            .filter((line: string) => line.length > 0);
        setItems(extractedItems.length > 0 ? extractedItems : ["Rice", "Dal"]);
    } else {
        setItems(["Rice", "Dal"]);
    }
  };
    ...
    const fetchData = async () => {
        try {
            ...
            if (data.success && data.data) {
                // API যদি Array দেয় তাহলে Array, না হলে সিঙ্গেল অবজেক্টকে Array তে কনভার্ট করবে
                const dishesArray = Array.isArray(data.data) ? data.data : [data.data];
                setAllDishes(dishesArray);
                if (dishesArray.length > 0) {
                    loadDishToState(dishesArray[0], "0");
                }
            }
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        // ★ API তে ID পাঠানো হচ্ছে যাতে ব্যাকএন্ড বুঝতে পারে কোন ডিশ আপডেট হবে
        const res = await fetch('/api/admin/daily-special', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                id: currentDishId, // ID Added
                name, price, items, 
        ...
        if (res.ok) { 
            toast.success(`${name} Updated & Poster Published! 🚀`);
            
            // লোকাল স্টেট আপডেট করা হচ্ছে
            const updatedDishes = [...allDishes];
            updatedDishes[parseInt(activeTabIndex)] = {
                 ...updatedDishes[parseInt(activeTabIndex)],
                 name, price, inStock
            };
    ...
    <div className="w-full pb-10 flex flex-col gap-6">
        <div className="flex items-center gap-3 bg-white p-4 rounded-xl border shadow-sm">
            ...
            <div>
                ...
                <p className="text-sm text-muted-foreground">Manage your 3 daily special dishes.</p>
            </div>
        </div>

        {/* ★ Tabs for 3 Dishes ★ */}
        <Tabs value={activeTabIndex} onValueChange={(val) => {
            if(allDishes[parseInt(val)]) loadDishToState(allDishes[parseInt(val)], val);
        }} className="w-full">
            <TabsList className="grid w-full grid-cols-3 h-12 items-center bg-white border shadow-sm">
                <TabsTrigger value="0" className="data-[state=active]:bg-primary data-[state=active]:text-white">Dish 1</TabsTrigger>
                <TabsTrigger value="1" disabled={!allDishes[1]} className="data-[state=active]:bg-primary data-[state=active]:text-white">Dish 2</TabsTrigger>
                <TabsTrigger value="2" disabled={!allDishes[2]} className="data-[state=active]:bg-primary data-[state=active]:text-white">Dish 3</TabsTrigger>
            </TabsList>
                ...
                <Card className="border-0 shadow-md w-full">
                    <CardContent className="p-4 sm:p-6 space-y-6">
                        ...
                        <Button onClick={handleSave} className="w-full h-12 text-lg shadow-lg shadow-primary/20" disabled={isSaving}>
                            {isSaving ? <Loader2 className="h-5 w-5 animate-spin mr-2" /> : <Save className="h-5 w-5 mr-2" />} Publish {name}
                        </Button>
                    </CardContent>
                </Card>

                {/* Right Side: Interactive Poster with Undo Button inside */}
